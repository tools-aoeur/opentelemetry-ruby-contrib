...
describe OpenTelemetry::Instrumentation::Net::LDAP::Instrumentation do
  ...
  describe '#request' do
    ...
    it 'after request with success code' do
      Net::LDAP.get('example.com', '/success')
    ...
    it 'after request with failure code' do
      Net::LDAP.post(URI('http://example.com/failure'), 'q' => 'ruby')
    ...
    it 'after request timeout' do
      expect do
        Net::LDAP.get(URI('https://example.com/timeout'))
  ...
  describe '#connect' do
    it 'emits span on connect' do
      ...
      TCPServer.open('localhost', 0) do |server|
        ...
        http = Net::LDAP.new(uri.host, uri.port)
        http.read_timeout = 0
        _(-> { http.request(Net::LDAP::Get.new(uri.request_uri)) }).must_raise(Net::ReadTimeout)
      end
    ...
    it 'captures errors' do
      ...
      http = Net::LDAP.new(uri.host, uri.port)
      _(-> { http.request(Net::LDAP::Get.new(uri.request_uri)) }).must_raise
    ...
    it 'emits an HTTP CONNECT span when connecting through an SSL proxy' do
      ...
      begin
        Net::LDAP.start(uri.host, uri.port, proxy_uri.host, proxy_uri.port, 'proxy_user', 'proxy_pass', use_ssl: true) do |http|
          http.get('/')
        end
    ...
    it 'emits a "connect" span when connecting through an non-ssl proxy' do
      ...
      # rubocop:disable Lint/SuppressedException
      begin
        Net::LDAP.start(uri.host, uri.port, proxy_uri.host, proxy_uri.port, 'proxy_user', 'proxy_pass', use_ssl: false) do |http|
          http.get('/')
        end
      rescue StandardError
      end
